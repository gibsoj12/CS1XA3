...
identifier = many1 $ alphaNum -- #TODO replace me
...
item = do  {key <- identifier;
            symbol ":";
            value <- integer;
            return (key,value) } -- #TODO replace me
...
items p = do { things <- sepBy p (symbol ",");
                return things } -- #TODO replace me
