...
{-|
Module      : ExprEval
Description : Contains instances for evaluation and simplification of the Expr data type
...
class EvalExpr a where
    ...
    neg :: Expr a -> Expr a
    neg e           = simplify (Map.fromList []) $ Neg e
    (!*) :: Expr a -> Expr a -> Expr a
    e1 !* e2        = simplify (Map.fromList []) $ Mult e1 e2
    sine :: Expr a -> Expr a
    sine e          = simplify (Map.fromList []) $ Sine e
    cosine :: Expr a -> Expr a
    cosine e        = simplify (Map.fromList []) $ Cosine e
    ln :: Expr a -> Expr a
    ...
    val x           = Const x
    var :: String -> Expr a
    var x           = simplify (Map.fromList []) $ Var x
...
instance (ForceFit a) => EvalExpr a where
    ...
    simplify vrs (Mult e1 e2)   = 
        ...
        in case (simpleE1, simpleE2) of
            ...
            (ex1, Mult ex2 (Inv ex3))       -> case simplify vrs ex1 == simplify vrs ex3 of
                                                    True        -> simplify vrs ex2
                                                    False       -> Mult (ex1) (Mult ex2 (Inv ex3))
            (Mult ex2 (Inv ex3), ex1)       -> case simplify vrs ex3 == simplify vrs ex1 of
                                                    True        -> simplify vrs ex2
                                                    False       -> Mult (ex1) (Mult ex2 (Inv ex3))
    ...
    simplify vrs (Inv e)     =
        ...
        in case simple1 of
            (Inv e)         -> simplify vrs e --Inverse of inverse is just the expression
            (Const x)       -> case eval vrs $ Inv (Const x) of
                                    AValue r    -> Const r
    ...
    simplify vrs (Lawg e1 e2)   =
        ...
        in case (simple1,simple2) of
            ...
            (Const x, Const y)      -> case x == y of
                                            True    -> Const 1
                                            False   -> Lawg (Const x) (Const y)
            ...
            (Var x, Var y)          -> case x == y of
                                            True    -> Const 1
                                            False   -> Lawg (Var x) (Var y)
            (e, (Inv e1'))          -> Neg $ Lawg e e1' -- Just bring the negative out
            (e1',e2')               -> case simplify vrs e1' == simplify vrs e2' of
                                            True    -> Const 1
                                            False   -> Lawg e1' e2'
